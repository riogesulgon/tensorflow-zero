...
model = tf.keras.Sequential([
    tf.keras.layers.Dense(50, activation='relu'), # relu - rectified linear unit. Activation function maps input values to output values
...
model.compile(loss=tf.keras.losses.mae,
              optimizer=tf.keras.optimizers.Adam(learning_rate=0.01), # Adam is a variant of SGD
              metrics=['mae']) # metrics is a list of metrics to be evaluated by the model during training and testing
...
# print expanded
print(expanded)
model.fit(expanded, Y, epochs=300) # training the model for 100 epochs
